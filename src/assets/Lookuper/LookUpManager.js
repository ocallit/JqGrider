function LookUpManager(categoria, element, options, catego) {
    if (!(this instanceof Catego_manager))
        return new Catego_manager(LookUpManager, element, options);
    ...
    function _handleAdd($dialog) {
        ...
        if(_yaExiste(newName, null)) {
            showErrorDialog('Ya existe: ' + newName);
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showErrorDialog(response.error || 'Error adding category');
                }
            },
            error: function() {
                showErrorDialog('Error communicating with server');
    ...
    function fetchData() {
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    var catego = response;
                    initializeManager(catego_value);
                } else {
                    showErrorDialog(response.error || 'Error fetching data');
                }
            },
            error: function (xhr, status, error) {
                showErrorDialog('Error communicating with the server: ' + error);
            }
        });
    }
